    ...
    fn step(
        ...
        for id in &self.ids {
            ...
            if let Some(robot) = &world.allies_bot.get(id) {
                ...
                if *id == KEEPER_ID {
                    action_wrapper.push(*id, MoveTo::new(world.geometry.ally_goal.line.center(), orientation, 0.0, false, None, true, vec![]));
                } else {
                    ...
                    action_wrapper.push(*id, MoveTo::new(target, orientation, 0.0, false, None, true, vec![]));
                    i += 1;
                }
            }
        }
