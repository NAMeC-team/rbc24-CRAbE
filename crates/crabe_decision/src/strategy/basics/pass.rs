...
pub fn pass(
    ...
    if passing_trajectory_will_land && dot_with_ball > 0.95{
        ...
        return MoveTo::new(ball_position, vectors::angle_to_point(robot_position,receiver.pose.position), 1.,  true, kick, true, vec![]);
    }
    MoveTo::new(behind_ball_position, vectors::angle_to_point(robot_position, receiver.pose.position), 0., false, None, true, vec![])
}