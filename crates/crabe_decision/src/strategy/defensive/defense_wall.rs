...
impl DefenseWall {
    ...
    pub fn oscillate(
        ...
        for id in self.ids.clone() {
            action_wrapper.push(id, MoveTo::new(pos, 0., 0., false, None, false, vec![]));
        }
        false
    }
                ...
                if distance_to_ball < KICK_RANGE + world.geometry.robot_radius + world.geometry.ball_radius {
                    if let Some(closest_bot_to_ball) = closest{
                        if closest_bot_to_ball.id == robot.id {
                            ...
                            action_wrapper.push(robot.id, MoveTo::new(ball_pos, ball_orientation, 0., true, Some(Kick::StraightKick { power: 4. }), false, self.ids.clone()));
                        }else {
                            action_wrapper.push(robot.id, MoveTo::new(pos_on_penalty_line, orientation, 0., false, None, true, self.ids.clone()));
                        }
                    }else {
                        action_wrapper.push(robot.id, MoveTo::new(pos_on_penalty_line, orientation, 0., false, None, true, self.ids.clone()));
                    }
