...
impl Strategy for GoalKeeper {
    ...
    fn step(
        ...
        if let Some(ball) = &world.ball{
            ...
            if let Some(intersection) = self.follow_velocity_trajectory(ball, world){
                ...
                if closests_receivers.len() == 0{
                    ...
                    action_wrapper.push(robot.id, MoveTo::new(ball_position, vectors::angle_to_point(robot.pose.position, ball_position), 0.0, true, kick, false, vec![]));
                    return false;
                }
                for receiver in closests_receivers.iter(){
                    if object_in_bot_trajectory(world, self.id, receiver.pose.position, false, false, true).len() == 0{
                        ...
                        action_wrapper.push(robot.id, MoveTo::new(ball_position, vectors::angle_to_point(robot.pose.position, ball_position), 0.0, true, Some(Kick::StraightKick { power: 4. }), false, vec![]));
                        return false;
                    }
                }
        ...
        action_wrapper.push(self.id, MoveTo::new(position_target, orientation, 0., false, None, false, vec![]));
        false
    }
